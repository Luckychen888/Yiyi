...
router.post('/letter', async (req, res) => {
  try {
    const db = getDB(req);
    const { coupleId, title, content, images, voiceUrl, fromId, fromName, fromAvatar, toId, openAt, musicUrl, musicName, template } = req.body;
    const letterId = generateId('letter');
    
    await db.query(
      `INSERT INTO letters (id, couple_id, title, content, images, voice_url, music_url, music_name, from_id, from_name, from_avatar, to_id, open_at, template, created_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [letterId, coupleId, title, content || null, JSON.stringify(images || []), voiceUrl || null, musicUrl || null, musicName || null, fromId, fromName, fromAvatar, toId || null, openAt || null, template || null]
    );
